...
namespace fix { namespace util {
	...
	constexpr bool safe_less(T t, S s)
	{
		...
		return
			(std::is_signed<T>::value && std::is_signed<S>::value) ? (st(t) < st(s)) : (
				(std::is_unsigned<T>::value && std::is_unsigned<S>::value) ? (ut(t) < ut(s)) : (
		...
		struct fitting_integral_type {
			...
			using type =
				conditional_t<
					...
					conditional_t<
						sizeof(A)==sizeof(B),
						make_signed_if_integral_t<promoted_type_t<larger_type_t<A,B>>>,
