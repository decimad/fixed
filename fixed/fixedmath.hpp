...
namespace fix
{
    ...
    namespace util
    {
        ...
        template <typename T>
        constexpr enable_if_integral_t<T, make_unsigned_if_integral_t<T>> abs2(T value)
        {
        ...
        template <typename T>
        constexpr enable_if_floating_point_t<T, T> abs2(T value)
        {
        ...
        template <typename T>
        constexpr enable_if_t<std::is_floating_point<T>::value || std::is_signed<T>::value, T> abs(T value)
        {
            return (value >= 0) ? value : -value;
        }

        template <typename T>
        constexpr enable_if_t<std::is_integral<T>::value && !std::is_signed<T>::value, T> abs(T value)
        {
            return value;
        }

        template <typename T>
        constexpr enable_if_t<std::is_integral<T>::value && std::is_signed<T>::value, T> safe_abs(T value)
        {
            return abs<T>(clamp<T>(value, -std::numeric_limits<T>::max(), std::numeric_limits<T>::max()));
        }

        template <typename T>
        constexpr enable_if_t<std::is_unsigned<T>::value || std::is_floating_point<T>::value, T> safe_abs(T value)
        {
            return abs(value);
        }
        ...
        template <typename T>
        constexpr enable_if_integral_t<T, T> exp2(unsigned int exponent)
        {
            return T(1) << exponent;
        }

        template <typename T>
        constexpr enable_if_floating_point_t<T, T> exp2(unsigned int exponent)
        {
        ...
        template <typename T>
        constexpr enable_if_integral_t<T, int> log2_floor(T value)
        {
            return (value <= 1) ? 0 : (1 + log2_floor(value >> 1));
        }

        template <typename T>
        constexpr enable_if_integral_t<T, int> log2_ceil(T value)
        {
            return (value <= 1) ? 0 : (((value & 1) && (value > 1)) ? (2 + log2_floor(value >> 1)) : (1 + log2_ceil(value >> 1)));
        }
        ...
        namespace detail
        {
            ...
            {
                return (value == T(2)) ? 1 : ((value > T(2)) ? (1 + log2_floor_float(value / 2)) : 0);
            }

            template <typename T>
            constexpr int log2_ceil_float(T value)
            {
                return (value == T(2)) ? 1 : ((value > T(2)) ? (1 + log2_ceil_float(value / 2)) : 1);
            }

        } // namespace detail

        template <typename T>
        constexpr enable_if_floating_point_t<T, int> log2_floor(T value)
        {
            return (value == T(1)) ? 0 : ((value >= T(1)) ? detail::log2_floor_float(value) : (-detail::log2_ceil_float(T(1) / value)));
        }

        template <typename T>
        constexpr enable_if_floating_point_t<T, int> log2_ceil(T value)
        {
            return (value == T(1)) ? 0 : ((value > T(1)) ? detail::log2_ceil_float(value) : (-detail::log2_floor_float(T(1) / value)));
        }

        template <typename T>
        constexpr enable_if_integral_t<T, T> round(T value)
        {
            return value;
        }

        template <typename T>
        constexpr enable_if_floating_point_t<T, T> round(T value)
        {
        ...
        constexpr enable_if_integral_t<T, T> ceil(T value, int digits)
        {
            return (NoMask ? (value) : (value & ~bitmask<T>(digits))) + ((value & bitmask<T>(digits)) ? (T(1) << digits) : 0);
        }

        template <bool NoMask = false, typename T>
        constexpr enable_if_integral_t<T, T> floor(T value, int digits)
        {
        ...
        {
            return static_cast<T>((static_cast<largest_signed_type>(value) == value || value > 0)
                                      ? static_cast<largest_signed_type>(value)
                                      : (static_cast<largest_signed_type>(value) - 1));
        }
        ...
        template <bool NoMask = false, typename T>
        constexpr enable_if_integral_t<T, T> towards_zero(T value, int digits)
        {
            return (value > 0) ? floor<NoMask>(value, digits) : ceil<NoMask>(value, digits);
        }

        template <typename T>
        constexpr enable_if_floating_point_t<T, T> towards_zero(T value)
        {
            return (value > 0) ? floor(value) : ceil(value);
        }

        template <bool NoMask = false, typename T>
        constexpr enable_if_integral_t<T, T> towards_infinity(T value, int digits)
        {
            return (value >= 0) ? ceil<NoMask>(value, digits) : floor<NoMask>(value, digits);
        }

        template <typename T>
        constexpr enable_if_floating_point_t<T, T> towards_infinity(T value)
        {
            return (value >= 0) ? ceil(value) : floor(value);
        }

        template <bool NoMask = false, typename T>
        constexpr enable_if_integral_t<T, T> nearest_up(T value, int digits)
        {
            return ((value & bitmask<T>(digits)) >= (1 << (digits - 1))) ? ceil<NoMask>(value, digits) : floor<NoMask>(value, digits);
        }

        template <typename T>
        constexpr enable_if_floating_point_t<T, T> nearest_up(T value)
        {
            return (value - floor(value) >= 0.5) ? ceil(value) : floor(value);
        }

        template <bool NoMask = false, typename T>
        constexpr enable_if_integral_t<T, T> nearest_down(T value, int digits)
        {
            return ((value & bitmask<T>(digits)) <= (T(1) << (digits - 1))) ? floor<NoMask>(value, digits) : ceil<NoMask>(value, digits);
        }

        template <typename T>
        constexpr enable_if_floating_point_t<T, T> nearest_down(T value)
        {
        ...
        constexpr enable_if_floating_point_t<T, T> scaled_exp2(T value, int exponent)
        {
            return (exponent >= 0) ? (detail::floating_round_switch<Mode>::round(value * exp2<util::largest_unsigned_type>(exponent)))
                                   : (value / exp2<util::largest_unsigned_type>(-exponent));
        }

        template <RoundModes Mode = RoundModes::Floor, typename T>
        constexpr enable_if_integral_t<T, T> scaled_exp2(T value, int exponent)
        {
            return (exponent >= 0) ? static_cast<T>(std::make_unsigned_t<T>(value) << exponent)
        ...
        namespace detail
        {
            ...
            template <typename T>
            constexpr enable_if_t<std::is_signed<T>::value, bool> max_val(T value)
            {
                return value == bitmask<T>(sizeof(T) * 8 - 1);
            }

            template <typename T>
            constexpr enable_if_t<std::is_unsigned<T>::value, bool> max_val(T value)
            {
        ...
        template <typename T>
        constexpr enable_if_integral_t<T, int> integer_bits(T value)
        {
            return (value == T(0)) ? (1) : ((value > T(0)) ? detail::log2_ceil_incr(abs2(value)) : (log2_ceil(abs2(value)) + 1));
        }

        template <typename T>
        constexpr enable_if_floating_point_t<T, int> integer_bits(T value)
        {
        ...
        namespace detail
        {
            ...
            constexpr int range_bits2(T t, U u)
            {
                ...
                return max<int>(integer_bits(abs2(t)) - (is_neg(t) ? 1 : 0), integer_bits(abs2(u)) - (is_neg(u) ? 1 : 0))
                       + (any_neg(t, u) ? 1 : 0);
            }
        ...
        {
            return (s == 0) ? integer_bits(u) : ((u == 0) ? integer_bits(s) : detail::range_bits2(s, u));
        }
