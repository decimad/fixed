...
#define SC static constexpr
    ...
    namespace detail
    {
        ...
        struct promoted
        {
            using bare_type
                = util::conditional_t<sizeof(T) == 1, short,
                                      util::conditional_t<sizeof(T) == 2, int, util::conditional_t<sizeof(T) == 4, int64, int64>>>;
        ...
        struct fitting_type
        {
            using type = util::conditional_t<sizeof(T1) == sizeof(T2) && !(std::is_signed<T1>::value == std::is_signed<T2>::value),
                                             promoted_t<T1, true>, util::conditional_t<(sizeof(T1) > sizeof(T2)), T1, T2>>;
        };
        ...
        struct value_type
        {
          private:
            ...
            using raw_type = util::conditional_t<(Bits <= 8), int8,
                                                 util::conditional_t<(Bits <= 16), int16, util::conditional_t<(Bits <= 32), int32, int64>>>;
...
#define RANGE_FROM_VALS(A, B) value_range<detail::fitting_type_t<decltype(A), decltype(B)>, A, B>

    template <auto Start, auto End>
    using range_from_vals = value_range<detail::fitting_type_t<decltype(Start), decltype(End)>, Start, End>;
    ...
    namespace detail
    {
        ...
        {
            return Signed ? ValueType(util::bitmask<util::make_unsigned_t<ValueType>>(Bits - 1))
                          : ValueType(util::bitmask<ValueType>(Bits));
        }
        ...
        {
            using type
                = value_range<RangeT, util::saturate(RangeMin, LimitsMin, LimitsMax), util::saturate(RangeMax, LimitsMin, LimitsMax)>;
        };
    ...
    template <int I, int F, bool S = true, typename ValueType = detail::value_type_t<I + F, S>,
              typename Range = detail::auto_fixed_range<I + F, S>>
    class fixed;
    ...
    namespace detail
    {
        ...
        struct scaling_shift_values<fixed<I, F, S, ValueType, Range>, Shift, Rounding>
        {
            ...
            using precast_type = util::conditional_t<(sizeof(destination_value_type) > sizeof(ValueType)) && (Shift > 0),
                                                     destination_value_type, ValueType>;
            ...
            using result_type
                = fixed<I, F + Shift, S, destination_value_type,
                        value_range<destination_value_type, util::scaled_exp2<Rounding::value>(precast_type(Range::minval), Shift),
                                    util::scaled_exp2<Rounding::value>(precast_type(Range::maxval), Shift)>>;
        };
        ...
        struct literal_shift_values<fixed<I, F, S, ValueType, Range>, Shift, Rounding>
        {
            ...
            using result_type
                = fixed<I + Shift, F, S, destination_value_type,
                        value_range<destination_value_type, util::scaled_exp2<Rounding::value>(precast_type(Range::minval), Shift),
                                    util::scaled_exp2<Rounding::value>(precast_type(Range::maxval), Shift)>>;
        };
    ...
    constexpr typename detail::scaling_shift_values<fixed<I, F, S, T, RT>, Shift, RoundMode>::result_type scaling_shift(
        fixed<I, F, S, T, RT> val);
    ...
    class fixed
    {
      ...
      public:
        ...
        constexpr fixed(value_type value_) : value(value_)
        {
        }

        template <int OI, int OF, bool OS, typename OT, typename ORT>
        constexpr fixed(const fixed<OI, OF, OS, OT, ORT> &other) : value(static_cast<value_type>(util::scaled_exp2(other.value, F - OF)))
        {
        }
      ...
      public:
        ...
        {
            return fixed<integer_bits + ((!is_signed && std::is_signed<T>::value) ? 1 : 0), fractional_bits, std::is_signed<T>::value, T,
                         range_type>(value);
        }
        ...
        {
            // Need to be more sophisticated here... detect powers of 2 and such which make negative integers or fractions possible (for
            // constants)
            return fixed(static_cast<value_type>(util::scaled_exp2<RoundingWrapper::value>(value, -scaling)));
        }

        template <typename U, typename RoundingWrapper = rounding::floor>
        constexpr typename std::enable_if<std::is_floating_point<U>::value, U>::type to() const
        {
            return util::scaled_exp2<RoundingWrapper::value, U>(value, scaling);
        }

        template <typename U, typename RoundingWrapper = rounding::floor>
        constexpr typename std::enable_if<std::is_integral<U>::value, U>::type to() const
        {
            return util::scaled_exp2<RoundingWrapper::value, U>(value, scaling);
        }
    };
        ...
        fixed<I, F, S, T, RT> val)
    {
        ...
        return return_type(
            static_cast<return_value_type>(util::scaled_exp2<RoundMode::value>(static_cast<precast_type>(val.value), Shift)));
    }

    template <int Shift, typename RoundMode = rounding::floor, int I, int F, bool S, typename T, typename RT>
    constexpr typename detail::scaling_shift_values<fixed<I, F, S, T, RT>, Shift, RoundMode>::result_type sshift(fixed<I, F, S, T, RT> val)
    {
        return scaling_shift<Shift, RoundMode>(val);
    }

    template <int Shift, int I, int F, bool S, typename T, typename RT>
    constexpr fixed<I + Shift, F - Shift, S, T, RT> virtual_shift(fixed<I, F, S, T, RT> val)
    {
        return fixed<I + Shift, F - Shift, S, T, RT>(val.value);
    }

    template <int Shift, int I, int F, bool S, typename T, typename RT>
    constexpr fixed<I + Shift, F - Shift, S, T, RT> vshift(fixed<I, F, S, T, RT> val)
    {
        return fixed<I + Shift, F - Shift, S, T, RT>(val.value);
    }

    template <int Shift, typename RoundMode = rounding::floor, int I, int F, bool S, typename T, typename RT>
    constexpr typename detail::literal_shift_values<fixed<I, F, S, T, RT>, Shift, RoundMode>::result_type literal_shift(
        fixed<I, F, S, T, RT> val)
    {
        ...
        return return_type(util::scaled_exp2<RoundMode::value>(static_cast<precast_type>(val.value), Shift));
    }

    template <int Shift, typename RoundMode = rounding::floor, int I, int F, bool S, typename T, typename RT>
    constexpr typename detail::literal_shift_values<fixed<I, F, S, T, RT>, Shift, RoundMode>::result_type lshift(fixed<I, F, S, T, RT> val)
    {
    ...
    namespace detail
    {
        ...
        struct auto_type
        {
            ...
            using type = util::conditional_t<std::is_signed<A>::value || std::is_signed<B>::value, util::make_signed_t<intermediate>,
                                             util::make_unsigned_t<intermediate>>;
        };

        template <typename Args, typename Nom, typename Denom>
        struct div_struct;

        template <typename Args, typename NomT, int NomI, int NomF, bool NomS, typename NomR, typename DenT, int DenI, int DenF, bool DenS,
                  typename DenR>
        struct div_struct<Args, fixed<NomI, NomF, NomS, NomT, NomR>, fixed<DenI, DenF, DenS, DenT, DenR>>
        {
            ...
            {
                return result_type(static_cast<result_value_type>(scaling_shift<shift_nom, rounding_type>(nom).value
                                                                  / scaling_shift<shift_den, rounding_type>(den).value));
            }
        };
    } // namespace detail

    template <typename... Args, typename NomType, typename DenType,
              typename Test = util::enable_if_t<is_fixed<NomType>::value && is_fixed<DenType>::value>>
    ...
    namespace detail
    {
        ...
        using mul_result_type_t = detail::value_type_t<(sizeof(typename RangeA::value_type) + sizeof(typename RangeB::value_type)) * 8,
                                                       std::is_signed<typename RangeA::value_type>::value
                                                           || std::is_signed<typename RangeB::value_type>::value>;
        ...
        struct max_mul_result
        {
            ...
            SC result_type value = util::max(util::max(result_type(RangeA::maxval) * result_type(RangeB::maxval),
                                                       result_type(RangeA::maxval) * result_type(RangeB::minval)),
                                             util::max(result_type(RangeA::minval) * result_type(RangeB::maxval),
                                                       result_type(RangeA::minval) * result_type(RangeB::minval)));
        };
        ...
        struct min_mul_result
        {
            ...
            SC result_type value = util::min(util::min(result_type(RangeA::maxval) * result_type(RangeB::maxval),
                                                       result_type(RangeA::maxval) * result_type(RangeB::minval)),
                                             util::min(result_type(RangeA::minval) * result_type(RangeB::maxval),
                                                       result_type(RangeA::minval) * result_type(RangeB::minval)));
        };

        template <typename RangeA, typename RangeB>
        using mul_result_range_t
            = value_range<mul_result_type_t<RangeA, RangeB>, min_mul_result<RangeA, RangeB>::value, max_mul_result<RangeA, RangeB>::value>;
        ...
        struct mul_struct<Args, fixed<AI, AF, AS, AT, AR>, fixed<BI, BF, BS, BT, BR>>
        {
            ...
            static constexpr auto result_min
                = (assume_positive && range::minval < 0) ? range_value_type(0) : (util::shifted(range::minval, result_shift));
            ...
            static constexpr result_type mul(a_type a, b_type b)
            {
                return result_type(static_cast<typename result_type::value_type>(
                    util::scaled_exp2<rounding_type::value>(intermediate_type(scaling_shift<a_shift, rounding_type>(a).value)
                                                                * intermediate_type(scaling_shift<b_shift, rounding_type>(b).value),
                                                            result_shift)));
            }
        };
...
#define DEBUG_MUL(A, B, ...) ::fix::detail::mul_struct<::fix::meta::list<__VA_ARGS__>, std::decay_t<decltype(A)>, std::decay_t<decltype(B)>>
    ...
    namespace detail
    {
        ...
        struct sub_result_value_type<value_range<AT, amin, amax>, value_range<BT, bmin, bmax>>
        {
            ...
            using type = detail::value_type_t<util::max<int>(util::range_bits(amin, amax), util::range_bits(bmin, bmax)),
        ...
        struct sub_temporary_value_type<value_range<AT, amin, amax>, value_range<BT, bmin, bmax>>
        {
            ...
            using type = detail::value_type_t<util::max<int>(util::range_bits(amin, amax), util::range_bits(bmin, bmax)) + 1,
                                              util::safe_less(a_range::minval, b_range::maxval)>;
        };
        ...
        struct max_sub_result
        {
            ...
            SC res_type value = static_cast<res_type>(util::max(
        ...
        struct min_sub_result
        {
            ...
            SC res_type value = static_cast<res_type>(util::min(
        ...
        {
            using type = value_type_t<util::max(util::range_bits(amin, amax), util::range_bits(bmin, bmax)),
        ...
        {
            using type = value_type_t<util::max(util::range_bits(amin, amax), util::range_bits(bmin, bmax)) + 1,
                                      util::is_neg(amin) || util::is_neg(amax) || util::is_neg(bmin) || util::is_neg(bmax)>;
        };
        ...
        struct add_sub_struct<ArgList, fixed<AI, AF, AS, AT, AR>, fixed<BI, BF, BS, BT, BR>>
        {
            ...
            static constexpr int max_size
                = parsed_args::max_size_constrained ? parsed_args::max_size : util::max(sizeof(AT), sizeof(BT)) * 8;
            using result_range = typename parsed_args::result_range;
            ...
            constexpr static int a_shift_preliminary
                = f_constrained ? (f_constrained_bits - A::fractional_bits) : util::max(exponent_difference, 0);
            constexpr static int b_shift_preliminary
                = f_constrained ? (f_constrained_bits - B::fractional_bits) : util::max(-exponent_difference, 0);
            ...
            constexpr static int overshoot = util::max(util::max((A::low_bits + a_shift_preliminary + a_sign_extension) - max_size,
                                                                 (B::low_bits + b_shift_preliminary + b_sign_extension) - max_size),
                                                       0);

            // If custom-dest fraction bits lead to a negative offset, shift both up accordingly
            // constexpr static int undershoot = f_constrained ? (-util::min(A::radix_pos + a_shift_preliminary - overshoot -
            // result_range::fraction_bits, 0)) : (0);
            ...
            using auto_sub_result_range = value_range<sub_result_value_type_t<shifted_a_range, shifted_b_range>,
                                                      min_sub_result<shifted_a_range, shifted_b_range>::value,
                                                      max_sub_result<shifted_a_range, shifted_b_range>::value>;

            using auto_add_result_range = value_range<add_result_value_type_t<shifted_a_range, shifted_b_range>,
            ...
            using max_add_range = auto_fixed_range<util::min<int>(util::max<int>(shifted_a_range::bits, shifted_b_range::bits),
                                                                  auto_add_result_range::bits),
                                                   auto_add_result_range::is_signed>;
            using max_sub_range = auto_fixed_range<util::min<int>(util::max<int>(shifted_a_range::bits, shifted_b_range::bits),
                                                                  auto_sub_result_range::bits),
                                                   auto_sub_result_range::is_signed>;
            ...
            {
                return sub_result_type(sub_temporary_value_type(scaling_shift<a_shift, rounding_type>(a).value)
                                       - sub_temporary_value_type(scaling_shift<b_shift, rounding_type>(b).value));
            }

            constexpr static add_result_type add(A a, B b)
            {
                return add_result_type(add_temporary_value_type(scaling_shift<a_shift, rounding_type>(a).value)
                                       + add_temporary_value_type(scaling_shift<b_shift, rounding_type>(b).value));
            }
        };
    } // namespace detail
...
#define FIXED_RANGE_FROM_VALS(I, F, A, B)                                                                                                  \
    ::fix::value_range<::fix::detail::value_type_t<(I) + (F), ::fix::util::any_neg(A, B)>,                                                 \
                       ::fix::detail::to_fixed<I, F, ::fix::util::any_neg(A, B), ::fix::rounding::floor>(::fix::util::mixed_min(A, B)),    \
                       ::fix::detail::to_fixed<I, F, ::fix::util::any_neg(A, B), ::fix::rounding::ceil>(::fix::util::mixed_max(A, B))>

// fixed type of reals ranging from A to B with given fraction bits (precision)
#define FIXED_RANGE_P(A, B, Precision)                                                                                                     \
    ::fix::fixed_auto<::fix::util::range_bits(A, B), (Precision), ::fix::util::any_neg(A, B),                                              \
                      FIXED_RANGE_FROM_VALS(::fix::util::range_bits(A, B), Precision, A, B)>

// fixed type of reals ranging from A to B with given size
#define FIXED_RANGE(A, B, Size)                                                                                                            \
    ::fix::fixed_auto<::fix::util::range_bits(A, B), (Size - ::fix::util::range_bits(A, B)), ::fix::util::any_neg(A, B),                   \
                      FIXED_RANGE_FROM_VALS(::fix::util::range_bits(A, B), (Size - ::fix::util::range_bits(A, B)), A, B)>

// fixed type of integers ranging from A to B
#define FIXED_RANGE_I(A, B)                                                                                                                \
    ::fix::fixed_auto<::fix::util::range_bits(A, B), 0, ::fix::util::any_neg(A, B),                                                        \
                      FIXED_RANGE_FROM_VALS(::fix::util::range_bits(A, B), 0, A, B)>

// Real constant with given fraction bits
#define FIXED_CONSTANT_P(A, Precision) FIXED_RANGE_P(A, A, Precision)::from(A)

// Integer constant
#define FIXED_CONSTANT_I(A) FIXED_RANGE_P(A, A, 0)::from(A)

// Real constant with given size
#define FIXED_CONSTANT(A, Size) FIXED_RANGE(A, A, Size)::from(A)

// template<auto A, auto B>
// using range_from_limits;

template <auto A, auto B>
using fixed_integer = ::fix::fixed_auto<
    ::fix::util::range_bits(A, B), 0, ::fix::util::any_neg(A, B),

    ::fix::value_range<::fix::detail::value_type_t<::fix::util::range_bits(A, B), ::fix::util::any_neg(A, B)>,
                       ::fix::detail::to_fixed<::fix::util::range_bits(A, B), 0, ::fix::util::any_neg(A, B), ::fix::rounding::floor>(
                           ::fix::util::mixed_min(A, B)),
                       ::fix::detail::to_fixed<::fix::util::range_bits(A, B), 0, ::fix::util::any_neg(A, B), ::fix::rounding::ceil>(
                           ::fix::util::mixed_max(A, B))>>;
